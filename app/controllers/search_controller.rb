class SearchController < ApplicationController
  ...
  def occupation
    occupations = "Housewife,Farmer,Soldier/Police,Business,Teacher,Student,Health care worker,Preschool child,Driver,Craftsman,Prisoner,Mechanic,Security Guard,Office,Domestic worker,Not working,Other,Unknown".split(',')
    search_string = params[:value]
    @results = occupations.collect{|occupation| "<li>#{occupation}</li>" if occupation =~ /#{search_string}/i}.compact
    render :text => @results.join("\n")
  end
