class DrugOrder < OpenMRS
  ...
  def quantity_remaining_from_last_order
    ...
    return quantity_remaining if prescription_encounter.nil?

    prescription_encounter.observations.find_by_concept_name("Whole tablets remaining and brought to clinic").each do |observation|
      ...
      if observation.drug == self.drug || (self.drug.name.match(/Stavudine/) && observation.drug == Drug.find_by_name("Stavudine 30 Lamivudine 150"))
        quantity_remaining += observation.value_numeric
      end
    end
  ...
  def prescriptions
    return [] if prescription_encounter.nil?
    prescription_encounter.to_prescriptions.select do |prescription|
  ...
  def daily_consumption
    ...
    prescriptions.each do |prescription|
      if prescription.drug == self.drug && prescription.frequency.match(/Morning|Evening|Daily/)
        daily_consumption += prescription.dose_amount
      elsif prescription.frequency.match(/Weekly/) # Just an example
        daily_consumption += prescription.dose_amount/7.0
      end
    end
    ...
    if daily_consumption != 0
      ...
      # This will break for other types of prescriptions. Hopefully this is not a widespread problem.
      return 2
    end
  end
  ...
  def self.recommended_art_prescription(weight)
    regimens = Hash.new
    Regimen.all_combinations.each do |regimen|
      regimens[regimen.regimen] = Array.new if regimens[regimen.regimen].nil?
      regimens[regimen.regimen] << regimen.to_drug_order if weight >= regimen.min_weight && weight < regimen.max_weight
    end
    return regimens
  end
  ...
  def self.drug_orders_to_regimen(drug_orders = [])
    unless drug_orders.compact.blank?
  ...
  def self.given_drugs_dosage(drug_orders)
    unless drug_orders.blank?
      drug_total = Hash.new(0)
      drug_orders.each do |order|
        unless order.drug.name == "Insecticide Treated Net"
      ...
      drug_orders.each do |order|
        unless order.drug.name == "Insecticide Treated Net"
          ...
          prescriptions.each do |p|
            orders << "#{order.drug.name},#{p.frequency},#{p.dose_amount.to_f},#{drug_total[order.drug.name]}"
          end
  ...
  def prescription_encounter
    ...
    dispensation_date = Date.parse(dispensation_encounter.encounter_datetime.to_s)
    dispensation_encounter.patient.encounters.find_by_type_name_and_date("ART Visit", dispensation_date).last
  end

  # TODO: this method has a lot of apparent bugs. Fix & refactor!
  def self.patient_adherence(patient,visit_date=Date.today)
    ...
    drugs_dispensed_last_time = previous_art_drug_orders.map(&:drug)
    ...
    drugs_dispensed_last_time.each do |drug|
