class LabPanel < OpenMRS
  ...
  def self.test_name(test_types=nil)
    return find(:all,:conditions=>["rec_id IN (?)",test_types],:group=>"rec_id").collect{|n|n.short_name} rescue nil
  end

  def self.get_test_type(test_name)
    panel_id = find(:first,:conditions=>["short_name=?",test_name]).rec_id rescue nil
