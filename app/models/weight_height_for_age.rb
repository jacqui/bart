class WeightHeightForAge < ActiveRecord::Base
  ...
  def self.patient_height_weight_values(patient)
    return find(:all, :conditions =>["age_in_months =? and sex = ?", patient.age_in_months,(patient.gender == "Female"?0:1)]).first
  end

  def self.median_weight(patient)
    weight_values = patient_height_weight_values(patient)
