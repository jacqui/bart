...
class Success
  ...
  class << self
    ...
    def sent_alert=(value)
      @@sent_alert=value
    end

    def verify
      end_of_day_summary if Time.now.hour == 16 and Time.now.min == 0

      return if sent_recent_alert?
      should_have_a_login_screen
      should_have_3_mongrels
      should_not_run_hot
      should_have_free_memory
      should_have_free_disk_space
      should_have_more_than_ten_minutes_uptime
      if clinic_is_active?
        should_have_recent_encounter
        should_have_low_load_average
      end
    end
    ...
    def sent_recent_alert?(since = 10.minutes.ago)
    ...
    def clinic_is_active?(current_time = Time.now)
    ...
    def clinic_hours=(value = "7:30am-3:45pm")
      Success.set_global_property("clinic_hours",value)
    end

    def clinic_breaks=(value = "12:30pm-1:00pm,1:00pm-1:15pm")
...
protected
  ...
  class << self
    def should_have_recent_encounter(since = 5.minutes.ago)
      notify this_method.capitalize.gsub(/_/, " ")
      last_encounter_time = Encounter.find(:first, :order => 'encounter_id DESC').date_created
      if last_encounter_time < since
        alert "Last encounter occurred > five minutes ago (#{last_encounter_time.hour}:#{last_encounter_time.min})"
      end
    end

    def should_have_a_login_screen
      notify this_method.capitalize.gsub(/_/, " ")
      login_screen = `lynx --dump localhost`
      alert "No login screen available:\n #{login_screen}" unless login_screen.match(/Loading User Login/)
    end

    def should_have_3_mongrels
      notify this_method.capitalize.gsub(/_/, " ")
      alert("3 mongrels are not running:\npgrep mongrel\n#{`pgrep mongrel`}", end_of_log) unless `pgrep mongrel`.split(/\n/).length == 3 rescue false
    end

    def should_not_run_hot
      notify this_method.capitalize.gsub(/_/, " ")
      current_temp = `cat /proc/acpi/thermal_zone/*/temperature | head -1`.match(/\d+/).to_s.to_i rescue nil
      alert("Machine is running hot: #{current_temp}", end_of_log) if current_temp.blank? || current_temp > 55
    end

    def should_have_free_memory
      notify this_method.capitalize.gsub(/_/, " ")
      mem_free = `cat /proc/meminfo | grep MemFree`
      mem_free_amount = mem_free.match(/\d+/)[0].to_i
      alert("Machine is running out of memory: #{mem_free_amount}", end_of_log) if mem_free_amount < (96 * 1024) # 96 MB
    rescue
      alert "Could not check the free memory"
    end

    def should_have_free_disk_space
      notify this_method.capitalize.gsub(/_/, " ")
    ...
    def should_have_more_than_ten_minutes_uptime
      notify this_method.capitalize.gsub(/_/, " ")
      uptime = `cat /proc/uptime`
      uptime_seconds = uptime.split(/ +/)[0].to_f
      alert("System was rebooted at #{Time.now - uptime_seconds}", end_of_log) if uptime_seconds/60 < 10
    rescue
      alert "Could not check uptime"
    end

    def should_have_low_load_average
      notify this_method.capitalize.gsub(/_/, " ")
      load_average = `cat /proc/loadavg`
      fifteen_minute_load_average = load_average.split(/ +/)[2].to_f
      alert("System has a high load average over the past 15 minutes: #{fifteen_minute_load_average}", end_of_log) if fifteen_minute_load_average > 2
    rescue
    ...
    def end_of_day_summary
      ...
      subject = "Number of unique #{current_location} patients for today: = #{number_of_unique_patients_with_encounters}"
      message = ""
...
private
  ...
  class << self
    ...
    def current_location
      Location.current_location.name rescue "Unknown location"
    end
    ...
    def alert(subject, extended_message = "")
      @@sent_alert = true
      ...
      reset(DateTime.now.to_default_s)
      body = "#{subject} @ #{current_location} (#{current_ip_address}) at #{Time.now}\n#{extended_message}"
...
END_OF_MESSAGE
      ...
      smtp_server = global_property("smtp_server") || "localhost"
